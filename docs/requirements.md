...
このドキュメントは、プロジェクトの要件に関する詳細な情報を提供し、プロジェクトの成功を確保するために必要なガイダンスを提供します。要件定義書は、プロジェクトの目的、スコープ、機能、非機能要件、ユーザー要件、システム要件、セキュリティ要件、品質要件、保守とサポートに関する情報が包括的に記載されています。
...
3. ユーザーはログアウトできる。
4. ユーザーはテキストメッセージを投稿できる。メッセージはテキストフォーマットで、最大文字数制限がある（1000文字まで）。
5. ユーザーは投稿したメッセージを編集できる。編集時には、最大文字数制限が適用される。
6. ユーザーは投稿したメッセージを削除できる。
7. ユーザーは他のユーザーのメッセージに対してコメントを投稿できる。コメントはテキストフォーマットで、最大文字数制限がある（500文字まで）。
8. メッセージとコメントは投稿日時とともに表示され、ページネーションまたは無限スクロールで表示され、1ページあたりの表示件数を設定できる。
9. メッセージとコメントをキーワードやユーザー名を使用して検索できる機能を実装する。
10. 新しいメッセージやコメントが投稿された場合、他のユーザーにリアルタイムで通知される（WebSocketを利用）。
...
- データベース: ユーザーアカウント情報やメッセージデータを保存するためのデータベースとして、リレーショナルデータベース（PostgreSQL）を使用します。
...
3. **投稿コメントデータ**
   - コメントID: 一意の識別子（整数またはUUID）
   - メッセージID: コメントされるメッセージを識別するための外部キー（投稿メッセージデータとの関連）
   - ユーザーID: コメントの投稿者を識別するための外部キー（ユーザーアカウントデータとの関連）
   - コメント内容: テキスト（最大長さ: 500文字）
   - 投稿日時: 日時（タイムスタンプ）
